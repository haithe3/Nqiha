...
import { ArrowLeft, Check, Clock3, Download, Droplets, MapPin, Menu, Phone, Share2, ShieldCheck, Sparkles, Star, Truck, X, ChevronDown } from "lucide-react";
...
const bookingTimes = ["08:00", "08:30", "09:00", "09:30", "10:00", "10:30", "11:00", "11:30", "12:00", "12:30", "13:00", "13:30", "14:00", "14:30", "15:00", "15:30", "16:00", "16:30", "17:00", "17:30", "18:00", "18:30", "19:00"] as const;
...
export default function Home() {
  ...
  const [bookingForm, setBookingForm] = useState({ customerName: "", phone: "", address: "", bookingDate: new Date().toISOString().slice(0, 10), bookingTime: "08:00", paymentMethod: "cash" as "cash" | "cib" | "baridimob" });
  const availableTimesQuery = trpc.bookings.availableTimes.useQuery({ bookingDate: bookingForm.bookingDate }, { enabled: bookingOpen && step === 1, retry: false });
  const availableTimes = availableTimesQuery.data ?? bookingTimes;
  const createBookingMutation = trpc.bookings.create.useMutation();
  const visitMutation = trpc.analytics.visit.useMutation();
  useEffect(() => { if (sessionStorage.getItem("nqiha-visit-recorded") !== "1") { visitMutation.mutate(); sessionStorage.setItem("nqiha-visit-recorded", "1"); } }, []);
...
الرقم المرجعي: ${bookingReference ?? "سيظهر بعد التأكيد"}
...
الدفع: ${bookingForm.paymentMethod === "cash" ? "كاش" : bookingForm.paymentMethod === "cib" ? "CIB" : "بريدي موب"}`);
  const whatsappUrl = `https://wa.me/213540399326?text=${whatsappMessage}`;
  ...
  const selectMapLocation = (latitude: number, longitude: number) => {
    setLocation({ latitude, longitude });
    updateForm("address", `الموقع المثبت (${latitude.toFixed(5)}, ${longitude.toFixed(5)})`);
  };
  const downloadReceipt = () => {
    if (!bookingReference) return;
    const pdf = new jsPDF({ unit: "mm", format: "a5" });
    pdf.setFontSize(20); pdf.text("NQIHA - Booking Receipt", 18, 22);
    pdf.setFontSize(11);
    const rows = [
      ...
      ["Total", `${totalWithTravel} DZD`],
    ];
    rows.forEach(([label, value], index) => pdf.text(`${label}: ${value}`, 18, 38 + index * 10));
    pdf.save(`${bookingReference}.pdf`);
  };
  ...
  return (
    <main dir="rtl" className="min-h-screen overflow-hidden bg-[#f7fbfc] text-[#073b63]">
      <header className="fixed inset-x-0 top-0 z-40 border-b border-white/50 bg-white/85 backdrop-blur-xl">
        <div className="container flex h-[76px] items-center justify-between">
          <a href="#top" className="flex items-center gap-3" aria-label="نقيها">
            <div className="brand-mark logo-image"><img src="/assets/nqiha-logo-two-drops.png" alt="شعار نقيها بقطرتين متداخلتين" /></div>
          ...
          <nav className="site-nav hidden items-center gap-7 text-sm font-bold md:flex"><a href="#top">الرئيسية</a><a href="#services">خدماتنا</a><a href="#how">كيف نخدمك؟</a><a href="#contact">اتصل بنا</a><a href="/account">حسابي وحجوزاتي</a><a href="/track">تتبع حجزي</a></nav>
          <div className="flex items-center gap-3"><a href="tel:+213540399326" className="hidden items-center gap-2 text-sm font-bold text-[#537188] lg:flex"><Phone size={16} /> 0540399326</a><Button onClick={() => startBooking()} className="rounded-full bg-[#073b63] px-5 font-bold shadow-lg shadow-[#073b63]/15 hover:bg-[#0b527f]">احجز الآن <ArrowLeft className="mr-2" size={16} /></Button><button onClick={() => setMobileOpen(!mobileOpen)} className="rounded-full p-2 md:hidden" aria-label="القائمة">{mobileOpen ? <X /> : <Menu />}</button></div>
        </div>
        {mobileOpen && <div className="border-t border-[#e4eff2] bg-white px-6 py-5 md:hidden"><div className="flex flex-col gap-5 text-sm font-bold"><a href="#top" onClick={() => setMobileOpen(false)}>الرئيسية</a><a href="#services" onClick={() => setMobileOpen(false)}>خدماتنا</a><a href="#how" onClick={() => setMobileOpen(false)}>كيف نخدمك؟</a><a href="#contact" onClick={() => setMobileOpen(false)}>اتصل بنا</a><a href="/account" onClick={() => setMobileOpen(false)}>حسابي وحجوزاتي</a><a href="/track" onClick={() => setMobileOpen(false)}>تتبع حجزي</a></div></div>}
      </header>
      ...
      <footer id="contact" className="site-footer border-t border-[#dfecef] bg-white"><div className="container grid gap-10 py-14 sm:grid-cols-2 lg:grid-cols-4"><div><div className="flex items-center gap-3"><div className="brand-mark small logo-image"><img src="/assets/nqiha-logo-two-drops.png" alt="شعار نقيها بقطرتين متداخلتين" /></div><div><div className="font-black">نقيها</div><div className="text-xs text-[#7a919b]">النظافة تجيك</div></div></div><p className="mt-5 text-sm leading-7 text-[#78909b]">غسيل سيارات متنقل يصلك أينما كنت.</p></div><div><h3 className="footer-title">روابط</h3><div className="footer-links"><a href="#services">خدماتنا وأسعارنا</a><a href="#how">كيف نخدمك؟</a><a href="#contact">اتصل بنا</a><a href="/account">حسابي وحجوزاتي</a><a href="/track">تتبع حجزي</a></div></div><div><h3 className="footer-title">تواصل معنا</h3><div className="footer-links"><a href="tel:+213540399326"><Phone size={16} /><span dir="ltr" className="phone-number">0540399326</span></a><a href="https://wa.me/213540399326" target="_blank" rel="noreferrer"><Share2 size={16} /> واتساب</a></div></div><div><h3 className="footer-title">أوقات العمل</h3><div className="flex items-center gap-2 text-sm text-[#617d90]"><Clock3 size={18} className="text-[#1397a5]" /><span>كل يوم من <b dir="ltr" className="phone-number">08:00</b> إلى <b dir="ltr" className="phone-number">19:00</b></span></div></div></div><div className="border-t border-[#edf3f4] py-5 text-center text-xs text-[#8a9da6]">© 2025 نقيها. كل الحقوق محفوظة.</div></footer>

      {bookingOpen && <div className="modal-backdrop" onClick={() => setBookingOpen(false)}><div className="booking-modal" onClick={e => e.stopPropagation()}><div className="booking-progress-head"><span>الخطوة {step + 1} من 3</span><strong>{Math.round(progress)}%</strong></div><div className="booking-progress-track"><div className="booking-progress-fill" style={{ width: `${progress}%` }} /></div><button onClick={() => setBookingOpen(false)} className="absolute left-5 top-5 rounded-full p-2 text-[#76909b] hover:bg-[#eef7f8]" aria-label="إغلاق"><X size={20} /></button>{confirmed ? <div className="py-7">
            <div className="text-center"><div className="success-logo-wrap"><img src="/assets/nqiha-logo-two-drops.png" alt="شعار نقيها" className="success-logo" /></div><div className="success-icon"><Check size={34} /></div><h2 className="mt-5 text-2xl font-black">تم استلام حجزك بنجاح!</h2><p className="mx-auto mt-2 max-w-[320px] text-sm leading-7 text-[#6b8592]">احتفظ بالرقم المرجعي. سنتواصل معك قريباً لتأكيد الموعد.</p></div>
            ...
            <div className="mt-6 grid grid-cols-2 gap-3"><Button variant="outline" onClick={downloadReceipt} className="flex h-11 items-center justify-center gap-2 rounded-full border-[#35c3d0] px-3 text-xs font-black text-[#168b9d] hover:bg-[#effbfc]"><Download size={16} /> تحميل PDF</Button><Button variant="outline" onClick={shareBooking} className="flex h-11 items-center justify-center gap-2 rounded-full border-[#d5e6e9] px-3 text-xs font-black text-[#073b63] hover:bg-[#f1fafa]"><Share2 size={16} /> مشاركة</Button><a href={whatsappUrl} target="_blank" rel="noreferrer" className="flex h-11 items-center justify-center gap-2 rounded-full bg-[#20b85a] px-3 text-xs font-black text-white hover:bg-[#149b48]"><Share2 size={15} /> إرسال واتساب</a><Button variant="outline" onClick={editBooking} className="col-span-2 h-11 rounded-full border-[#35c3d0] font-bold text-[#168b9d] hover:bg-[#effbfc]">تعديل بيانات الحجز</Button><Button onClick={() => setBookingOpen(false)} className="col-span-2 h-11 rounded-full bg-[#073b63] px-7 font-bold">إغلاق</Button></div>
            <div className="rating-panel mt-5 rounded-2xl border border-[#e2eef0] bg-[#fbfefe] p-4 text-center"><div className="text-sm font-black text-[#073b63]">كيف كانت تجربتك معنا؟</div>{feedbackSent ? <p className="mt-2 text-xs font-bold text-[#168b9d]">شكراً لتقييمك، رأيك يهمنا.</p> : <><div className="mt-3 flex justify-center gap-1" role="group" aria-label="تقييم الخدمة">{[1,2,3,4,5].map(value => <button type="button" key={value} onClick={() => setRating(value)} className={`rating-star ${value <= rating ? "selected" : ""}`} aria-label={`${value} نجوم`}><Star size={23} fill="currentColor" /></button>)}</div><Button type="button" variant="outline" onClick={submitRating} disabled={!rating} className="mt-3 h-9 rounded-full border-[#35c3d0] px-5 text-xs font-black text-[#168b9d]">إرسال التقييم</Button></>}</div>
          </div> : <><div className="mb-7"><p className="section-kicker">حجز سريع</p><h2 className="mt-2 text-2xl font-black">احجز غسيل سيارتك</h2></div><div className="mb-8 flex items-center gap-2">{steps.map((s, i) => <div key={s} className="flex flex-1 items-center gap-2"><div className={`step-number ${i <= step ? "active" : ""}`}>{i + 1}</div><span className={`hidden text-xs font-bold sm:block ${i <= step ? "text-[#073b63]" : "text-[#9bb0ba]"}`}>{s}</span>{i < 2 && <div className={`h-px flex-1 ${i < step ? "bg-[#35c3d0]" : "bg-[#e1ecef]"}`} />}</div>)}</div><div key={step} className={`booking-step booking-step-${step} ${step > 0 ? "booking-step-forward" : ""}`}>{step === 0 && <div className="space-y-5"><div>{services.map(s => <button key={s.title} onClick={() => setSelectedService(s.title)} className={`booking-option ${selectedService === s.title ? "selected" : ""}`}><div><div className="font-black">{s.title}</div><div className="mt-1 text-xs text-[#7a909b]">{s.desc} · {s.time}</div></div><div className="font-black">{s.title === selectedService ? displayPrice : s.price}</div></button>)}</div><div><div className="mb-3 flex items-center justify-between"><span className="text-xs font-black text-[#527183]">نوع السيارة</span><span className="text-xs font-black text-[#1397a5]">السعر يتحدث تلقائيًا</span></div><div className="grid grid-cols-3 gap-2">{carTypes.map(car => <button key={car.label} onClick={() => setSelectedCar(car.label)} className={`car-option ${selectedCar === car.label ? "selected" : ""}`}><span className="font-black">{car.label}</span><small>{car.hint}</small></button>)}</div></div></div>}{step === 1 && <div className="grid gap-4"><label className="field-label">اختار اليوم<input type="date" value={bookingForm.bookingDate} onChange={event => updateForm("bookingDate", event.target.value)} /></label><label className="field-label">الوقت المناسب<select dir="rtl" value={availableTimes.includes(bookingForm.bookingTime as typeof bookingTimes[number]) ? bookingForm.bookingTime : ""} onChange={event => updateForm("bookingTime", event.target.value)} disabled={availableTimesQuery.isLoading || !availableTimes.length} className="booking-time-select"><option value="" disabled>{availableTimesQuery.isLoading ? "جارٍ تحميل الأوقات…" : "اختر وقتاً واحداً"}</option>{availableTimes.map(time => <option dir="ltr" key={time} value={time}>{time}</option>)}</select></label></div>}{step === 2 && <div className="grid gap-4"><label className="field-label">الاسم الكامل<input value={bookingForm.customerName} onChange={event => updateForm("customerName", event.target.value)} placeholder="مثال: محمد بن علي" /></label><label className="field-label">رقم الهاتف<input value={bookingForm.phone} onChange={event => updateForm("phone", event.target.value)} placeholder="05 xx xx xx xx" /></label><label className="field-label">الموقع أو العنوان<input value={bookingForm.address} onChange={event => updateForm("address", event.target.value)} placeholder="أين نجيك؟" /></label><div className="location-row"><button type="button" onClick={useMyLocation} className="location-button"><MapPin size={17} /> استخدم موقعي الحالي</button><span>{location ? `رسوم التنقل: ${travelFee} دج` : "اضغط على الخريطة لتحديد موقعك"}</span></div><MapView selectedLocation={location} onLocationSelect={selectMapLocation} /><div className="payment-box"><div className="mb-3 text-xs font-black text-[#527183]">طريقة الدفع</div><div className="grid grid-cols-3 gap-2">{([["cash", "كاش"], ["cib", "CIB"], ["baridimob", "بريدي موب"]] as const).map(([value, label]) => <button type="button" key={value} onClick={() => updateForm("paymentMethod", value)} className={`payment-option ${bookingForm.paymentMethod === value ? "selected" : ""}`}>{label}</button>)}</div><div className="mt-3 flex items-center justify-between border-t border-[#e5eff1] pt-3 text-sm"><span className="font-bold text-[#6c8490]">المجموع</span><strong className="text-lg text-[#073b63]">{totalWithTravel} دج</strong></div></div></div>}</div><div className="mt-8 flex gap-3">{step > 0 && <Button variant="outline" onClick={() => setStep(step - 1)} className="h-12 flex-1 rounded-full border-[#d5e6e9] font-bold">رجوع</Button>}<Button disabled={createBookingMutation.isPending || (step === 1 && (availableTimesQuery.isLoading || !bookingForm.bookingTime || !availableTimes.includes(bookingForm.bookingTime as typeof bookingTimes[number]))) || (step === 2 && (!bookingForm.customerName.trim() || !/^(0|\+213)[5-7][0-9]{8}$/.test(normalizeDigits(bookingForm.phone).replace(/[\s-]/g, "")) || !bookingForm.address.trim()))} onClick={() => step < 2 ? setStep(step + 1) : submitBooking()} className="h-12 flex-1 rounded-full bg-[#073b63] font-bold hover:bg-[#0b527f]">{createBookingMutation.isPending ? "جارٍ تأكيد الحجز…" : step < 2 ? "التالي" : "تأكيد الحجز"}<ArrowLeft className="mr-2" size={17} /></Button></div>{createBookingMutation.error && <p role="alert" className="mt-3 text-center text-xs font-bold text-red-600">{createBookingMutation.error.message}</p>}</>}</div></div>}
    </main>
  );
}

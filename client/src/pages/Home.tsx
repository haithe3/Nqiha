...
import { ArrowLeft, Check, Clock3, Droplets, MapPin, Menu, Phone, ShieldCheck, Sparkles, Star, Truck, X, ChevronDown } from "lucide-react";
import { Button } from "@/components/ui/button";
import { trpc } from "@/lib/trpc";
import { MapView } from "@/components/Map";

const services = [
  { title: "الغسيل السريع", desc: "غسيل خارجي بلمعة منعشة", price: "800 دج", time: "30 دقيقة", icon: Droplets, accent: "aqua" },
  { title: "الباقة الكاملة", desc: "خارجي + داخلي + تعقيم", price: "1,500 دج", time: "60 دقيقة", icon: Sparkles, accent: "navy", featured: true },
  { title: "العناية الفاخرة", desc: "تفصيل شامل ولمعان عميق", price: "2,500 دج", time: "90 دقيقة", icon: Star, accent: "gold" },
];
...
export default function Home() {
  ...
  const [bookingForm, setBookingForm] = useState({ customerName: "", phone: "", address: "", bookingDate: "2025-06-18", bookingTime: "10:00", paymentMethod: "cash" as "cash" | "cib" | "baridimob" });
...
الدفع: ${bookingForm.paymentMethod === "cash" ? "كاش" : bookingForm.paymentMethod === "cib" ? "CIB" : "بريدي موب"}`);
  ...
  const submitBooking = async () => {
    await createBookingMutation.mutateAsync({ customerName: bookingForm.customerName, phone: bookingForm.phone, service: selectedService, carType: selectedCar, bookingDate: bookingForm.bookingDate, bookingTime: bookingForm.bookingTime, address: bookingForm.address, latitude: location ? String(location.latitude) : undefined, longitude: location ? String(location.longitude) : undefined, travelFee, totalPrice: totalWithTravel, paymentMethod: bookingForm.paymentMethod });
    setConfirmed(true);
  };
      ...
      <header className="fixed inset-x-0 top-0 z-40 border-b border-white/50 bg-white/85 backdrop-blur-xl">
        <div className="container flex h-[76px] items-center justify-between">
          <a href="#top" className="flex items-center gap-3" aria-label="نقيها">
            <div className="brand-mark"><Droplets size={21} fill="currentColor" /></div>

...
import { ArrowLeft, Check, Clock3, Download, Droplets, Mail, MapPin, Menu, Phone, Share2, ShieldCheck, Sparkles, Star, Truck, Volume2, VolumeX, X, ChevronDown } from "lucide-react";
...
الدفع: ${bookingForm.paymentMethod === "cash" ? "كاش" : bookingForm.paymentMethod === "cib" ? "CIB" : "بريدي موب"}`);
  ...
  return (
    <main dir="rtl" className="min-h-screen overflow-hidden bg-[#f7fbfc] text-[#073b63]">
      <header className="fixed inset-x-0 top-0 z-40 border-b border-white/50 bg-white/85 backdrop-blur-xl">
        <div className="container flex h-[76px] items-center justify-between">
          <a href="#top" className="flex items-center gap-3" aria-label="نقيها">
            <div className="brand-mark logo-image"><img src="/nqiha-logo.png" alt="شعار نقيها بقطرتين متداخلتين" /></div>
          ...
          <div className="flex items-center gap-3"><a href="tel:+213540399326" className="hidden items-center gap-2 text-sm font-bold text-[#537188] lg:flex" dir="ltr"><Phone size={16} /> 0540 39 93 26</a><Button onClick={() => startBooking()} className="rounded-full bg-[#073b63] px-5 font-bold shadow-lg shadow-[#073b63]/15 hover:bg-[#0b527f]">احجز الآن <ArrowLeft className="mr-2" size={16} /></Button><button onClick={() => setMobileOpen(!mobileOpen)} className="rounded-full p-2 md:hidden" aria-label="القائمة">{mobileOpen ? <X /> : <Menu />}</button></div>
        </div>
        {mobileOpen && <div className="border-t border-[#e4eff2] bg-white px-6 py-5 md:hidden"><div className="flex flex-col gap-5 text-sm font-bold"><a href="#services" onClick={() => setMobileOpen(false)}>خدماتنا</a><a href="#how" onClick={() => setMobileOpen(false)}>كيف نخدمك؟</a><a href="#why" onClick={() => setMobileOpen(false)}>لماذا نقيها؟</a><div className="mt-1 flex flex-col gap-4 border-t border-[#e4eff2] pt-5 text-[#537188]"><a href="tel:+213540399326" className="flex items-center gap-2" dir="ltr" onClick={() => setMobileOpen(false)}><Phone size={16} /> 0540 39 93 26</a><a href="mailto:contact@nqiha.com" className="flex items-center gap-2" dir="ltr" onClick={() => setMobileOpen(false)}><Mail size={16} /> contact@nqiha.com</a></div></div></div>}
      </header>
      ...
      <section id="top" className="hero-grid relative pt-[76px]">
        <div className="container grid min-h-[650px] items-center gap-12 py-20 lg:grid-cols-[1.04fr_.96fr]">
          <div className="relative z-10 max-w-[650px]">
            ...
            <div className="mt-10 grid grid-cols-3 gap-2.5 text-xs font-bold text-[#617d90] sm:flex sm:flex-wrap sm:gap-6"><span className="flex flex-col items-center gap-2 rounded-2xl border border-[#d3e8ec] bg-white px-2 py-3 text-center shadow-sm sm:flex-row sm:rounded-none sm:border-0 sm:bg-transparent sm:px-0 sm:py-0 sm:text-right sm:shadow-none"><ShieldCheck size={18} className="text-[#21a9b6]" /> منتجات آمنة</span><span className="flex flex-col items-center gap-2 rounded-2xl border border-[#d3e8ec] bg-white px-2 py-3 text-center shadow-sm sm:flex-row sm:rounded-none sm:border-0 sm:bg-transparent sm:px-0 sm:py-0 sm:text-right sm:shadow-none"><Clock3 size={18} className="text-[#21a9b6]" /> نلتزم بالموعد</span><span className="flex flex-col items-center gap-2 rounded-2xl border border-[#d3e8ec] bg-white px-2 py-3 text-center shadow-sm sm:flex-row sm:rounded-none sm:border-0 sm:bg-transparent sm:px-0 sm:py-0 sm:text-right sm:shadow-none"><Star size={18} fill="#f3b94b" className="text-[#f3b94b]" /> +4.9 تقييم العملاء</span></div>
            <div className="relative mt-10 overflow-hidden rounded-[32px] bg-gradient-to-br from-[#0b5079] via-[#073b63] to-[#062d4e] p-7 shadow-xl shadow-[#073b63]/20 lg:hidden">
              <div className="absolute inset-0 opacity-20 dot-pattern" />
              <div className="relative flex items-center gap-5">
                <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-full bg-[#35c3d0]/20 text-[#5ee4ed]"><Droplets size={44} strokeWidth={1.2} /></div>
                <div><div className="text-lg font-black text-white">نقيها توصلك</div><div className="mt-1 text-sm text-[#bceef1]">نظافة لامعة… بدون ما تتحرك</div></div>
              </div>
              <div className="relative mt-6 flex items-center gap-3 rounded-2xl bg-white/95 p-3"><div className="rounded-xl bg-[#e8fafb] p-2 text-[#159aaa]"><Truck size={22} /></div><div><div className="text-xs font-bold text-[#6c8290]">نجيك حتى للدار</div><div className="mt-0.5 text-sm font-black text-[#073b63]">خدمة متنقلة 100%</div></div></div>
            </div>
          </div>
      ...
      <footer className="border-t border-[#dfecef] bg-white"><div className="container flex flex-col gap-7 py-10 md:flex-row md:items-center md:justify-between"><div className="flex items-center gap-3"><div className="brand-mark small logo-image"><img src="/nqiha-logo.png" alt="شعار نقيها بقطرتين متداخلتين" /></div><div><div className="font-black">نقيها</div><div className="text-xs text-[#7a919b]">النظافة تجيك</div></div></div><div className="text-sm text-[#7a919b]">© 2025 نقيها. كل الحقوق محفوظة.</div><div className="flex flex-col gap-2 text-sm font-bold text-[#073b63]"><a href="tel:+213540399326" className="flex items-center gap-2" dir="ltr"><Phone size={16} /> 0540 39 93 26</a><a href="mailto:contact@nqiha.com" className="flex items-center gap-2" dir="ltr"><Mail size={16} /> contact@nqiha.com</a></div></div></footer>
